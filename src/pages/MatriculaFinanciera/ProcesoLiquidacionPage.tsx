...
export function ProcesoLiquidacionPage() {
  ...
  return <ModuleLayout title="Tablero de matrícula financiera"><div className="mf-page"><BackButton to="/matricula/financiera">Volver a procesos</BackButton><Aviso error={op.error || consulta.error} message={op.message} />{op.error && <p>Si una operación de envío falló, revisa el resultado parcial antes de volver a enviar; no se reintenta automáticamente.</p>}{progreso && <p role="status">{progreso}</p>}{resultado && <ResultadoOperacion titulo={resultado.titulo} omitidos={resultado.omitidos} codigos={codigos}>{resultado.mensaje}</ResultadoOperacion>}
    ...
    {proceso && <><header className="mf-page__intro"><div><span className={`mf-badge mf-badge--${proceso.estado.toLowerCase()}`}>{proceso.estado}</span><h1>Liquidaciones · {proceso.periodo}</h1><p>Recepción de respuestas habilitada hasta el {fechaColombia(proceso.fechaLimiteRespuesta)} · SMMLV {money(proceso.valorSmmlv)}</p></div><button disabled={blocked} className="mf-button mf-button--secondary" onClick={() => void op.run(() => exportarProceso(id), 'Excel descargado.')}>Exportar Excel</button></header>
      <section className="mf-summary" aria-label="Resumen del proceso">{Object.entries(proceso.resumen).filter(([key]) => key !== 'conAlertas').map(([key, value]) => <div className="mf-card" key={key}><strong>{value}</strong><span>{etiquetaResumen(key)}</span></div>)}</section>
      <details className="mf-card"><summary>Parámetros y fechas del proceso</summary><dl className="mf-values"><div><dt>Fuente SMMLV</dt><dd>{proceso.fuenteSmmlv || 'Sin fuente registrada'}</dd></div><div><dt>SMMLV</dt><dd>{money(proceso.valorSmmlv)}</dd></div><div><dt>Porcentaje de votación</dt><dd>{proceso.porcentajeVotacion}%</dd></div><div><dt>Porcentaje de salud</dt><dd>{proceso.porcentajeSalud}%</dd></div><div><dt>Fecha del primer envío de solicitudes</dt><dd>{fechaColombia(proceso.fechaEnvioSolicitudes)}</dd></div><div><dt>Fecha límite recepción respuestas</dt><dd>{fechaColombia(proceso.fechaLimiteRespuesta)}</dd></div></dl>{proceso.estado !== 'PUBLICADO' && <div className="mf-actions"><button type="button" disabled={blocked} className="mf-button mf-button--secondary" onClick={() => setEditar(!editar)}>Editar parámetros</button></div>}</details>
      <fieldset disabled={blocked} className="mf-fieldset mf-page">
        {allowed('convocar') && <div className="mf-actions"><button className="mf-button mf-button--secondary" onClick={() => setAgregar(!agregar)}>Agregar estudiante</button></div>}
        ...
        {allowed('convocar') && <section className="mf-card"><h2>Convocar estudiantes</h2><div className="mf-convocation-warning" role="note" aria-label="Advertencia antes de convocar"><span className="mf-convocation-warning__icon" aria-hidden="true">!</span><div><strong>Importante antes de convocar</strong><p>Al convocar, el sistema lista a todos los estudiantes activos (vigentes y nuevos). Revisa cuidadosamente cada estudiante y excluye desde su detalle a quien no corresponda <strong>antes de enviar las solicitudes</strong>. De lo contrario, se habilitará el proceso y se enviará correo a estudiantes que no deban participar.</p></div></div><div className="mf-actions"><button className="mf-button" onClick={() => void op.run(async () => { const result = await ejecutarAccionProceso(id, 'convocar', { incluirVigentes: true, incluirNuevos: true }); setResultado({ titulo: 'Convocatoria', mensaje: `${result.creadas} creadas · ${result.yaExistentes} ya existentes.`, omitidos: result.omitidos }); refresh() }, 'Convocatoria procesada.')}>{op.busy ? 'Convocando…' : 'Convocar'}</button></div></section>}
        ...
        <form className="mf-filters" onSubmit={e => { e.preventDefault(); filter({ texto: texto.trim() }) }}><label className="mf-filter--programa">Programa<select value={filtros.programaId ?? ''} onChange={e => filter({ programaId: e.target.value ? Number(e.target.value) : undefined })}><option value="">Todos los programas</option>{(programas.data ?? []).map(p => <option key={p.id} value={p.id}>{p.nombre}</option>)}</select></label><label>Estado<select value={filtros.estado ?? ''} onChange={e => filter({ estado: e.target.value as EstadoLiquidacion | '' })}><option value="">Todos</option><option value="PENDIENTE_RESPUESTA">Pendiente</option><option value="RESPONDIDA">Respondida</option><option value="LIQUIDADA">Liquidada</option><option value="NO_LIQUIDAR">No liquidar</option></select></label><label>Estudiante<input placeholder="Código o nombre" value={texto} onChange={e => setTexto(e.target.value)} /></label><button className="mf-button mf-button--secondary">Buscar</button></form>
        <Aviso error={programas.error} />
        {!consulta.loading && !consulta.error && <><div className="mf-table-wrap"><table className="mf-table"><thead><tr><th>Nombre</th><th>Código</th><th>Programa académico</th><th>Tipo de estudiante</th><th>Estado</th><th>Total</th><th>Acciones</th></tr></thead><tbody>{visibleRows.map(row => <tr key={row.id}><td>{row.nombreCompleto || 'Nombre no disponible'}</td><td><b>{row.codigoEstudiante}</b></td><td>{row.programa}</td><td>{row.tipoEstudiante}</td><td><span className={`mf-badge mf-badge--${row.estado.toLowerCase()}`}>{etiquetaEstadoLiquidacion(row.estado)}</span></td><td>{money(row.totalFinal)}</td><td><div className="mf-row-actions"><Link className="mf-button mf-button--secondary mf-button--table" to={`/matricula/financiera/procesos/${id}/liquidaciones/${row.id}`}>Ver detalle</Link><LiquidacionActions compact fila={row} proceso={proceso} onChange={refresh} /></div></td></tr>)}</tbody></table></div>{!rows.length && <p className="mf-empty">No hay liquidaciones para estos filtros.</p>}<Paginacion pagina={page} total={Math.ceil(rows.length / 20)} onChange={setPagina} /></>}
      </fieldset>
    </>}
  </div></ModuleLayout>
}
