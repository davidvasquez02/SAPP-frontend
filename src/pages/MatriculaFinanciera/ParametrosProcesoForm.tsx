...
export function ParametrosProcesoForm({ proceso, periodos = [], procesos = [], busy, onSave, onCancel }: ParametrosProcesoFormProps) {
  ...
  }}><fieldset disabled={busy} className="mf-form mf-fieldset"><div className="mf-form__heading"><h2>{proceso ? 'Editar parámetros' : 'Crear proceso'}</h2>{proceso ? <div className="mf-parameter-warning" role="note" aria-label="Consecuencias de guardar los parámetros"><span className="mf-parameter-warning__icon" aria-hidden="true">!</span><div><strong>Consecuencias de guardar cambios</strong><p>Guardar recalcula las filas sin valor final manual. Revisa los valores antes de exportar nuevamente.</p></div></div> : <p>Selecciona un periodo libre y configura los valores del proceso.</p>}{proceso?.baseSalud === 'MATRICULA' && <p className="mf-notice">Este proceso usa históricamente la matrícula como base de salud. Al guardar se aplicará SMMLV y el backend recalculará según las reglas actuales.</p>}</div>
