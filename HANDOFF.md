...
- Ajuste de paginación: el raster continuo ya no se corta cada 1056 px ocupando toda la hoja. Cada página reserva 89 px superiores y 83 px inferiores (aproximaciones a 2,35 cm y 2,2 cm a 96 dpi) y consume segmentos de 884 px de contenido. El padding vertical de `main` se retira durante el render para no duplicar el margen de la primera hoja; su padding horizontal permanece intacto.
...
- `npm run build` (2026-09-02, márgenes por página): PASS; TypeScript y rolldown-vite transformaron 233 módulos y generaron `dist/assets/index-CDQZMyK0.js` en 822 ms.
- `npx eslint src/modules/solicitudes/utils/htmlToPdf.ts` (2026-09-02, márgenes por página): PASS; npm emitió únicamente el warning conocido `Unknown env config "http-proxy"`.
