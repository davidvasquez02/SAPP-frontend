...
- Aprobación de solicitud: `PUT /api/sapp/solicitudesAcademicas/cambioEstado/{solicitudId}?siglaEstado=APROBADA&actaId={actaId}` sin body. `actaId` se obtiene de `GET /api/sapp/actas` y debe corresponder a la instancia del estado anterior (`tipoConsejo: false` para Comité, `true` para Consejo); las solicitudes OTRA pueden añadir `enviarConsejo=true`.
