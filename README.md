...
- April 6, 2026: en detalle de inscripción (coordinación/secretaría), al expandir “Documentos cargados” se dispara una sola vez `PUT /sapp/inscripcionAdmision/cambioEstadoVal/{inscripcionId}` cuando el estado actual está en `EN CONSTRUCCION`/`EN_CONSTRUCCION`; la llamada no bloquea la carga de documentos, muestra estado de actualización inline y permite reintento solo si falla.
