...
- **2026-09-04:** la creación y edición de períodos quedó dentro del módulo **Fechas** en `/fechas/periodos` (con `?periodoId={id}` para editar), por lo que el menú lateral mantiene seleccionado **Fechas** durante todo el flujo. El formulario ofrece **Atrás** hacia `/fechas` y el listado de períodos se pagina de a 4 registros; las convocatorias conservan páginas de 8. El listado consume `GET /sapp/periodoAcademico/withFechas` y la edición actualiza tanto el período como sus fechas de matrícula mediante los servicios existentes.
