...
- **Aspirante creation flow:** `CreateAspiranteModal` posts `/sapp/aspirante`, stores the returned `aspiranteId` + `inscripcionAdmisionId`, and then uploads selected documents **sequentially** via `POST /sapp/document` using base64 + SHA-256 helpers. The modal now loads trámite documents from backend (`GET /sapp/tramite/document?tipoTramiteId=1`) using `httpClient`, filters by `ADMISION_COORDINACION`, computes required items from backend `obligatorio`, preserves sequential uploads/retry behavior, and keeps a DEV-only template fallback if the fetch fails.
...
- Switched aspirante creation requirements to backend trámite documents (filtered by `ADMISION_COORDINACION`) with loading/error/empty states, retry action, and DEV-only fallback template.
