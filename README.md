...
- **2026-09-04:** el listado de períodos del módulo **Fechas** consume ahora `GET /sapp/periodoAcademico/withFechas`, presenta los rangos del período y de matrículas, pagina cada listado de a 8 registros y ofrece **Editar** en cada fila. La pantalla antes denominada “Gestionar períodos” se convirtió en **Crear período académico**: ya no contiene selector de períodos; al entrar desde **Editar** con `?periodoId={id}` precarga el formulario completo y permite actualizar tanto el período como sus fechas de matrícula mediante los servicios existentes.
