...
- **2026-09-08:** **Generar informe** en **Informes a dependencias** ahora trata `POST /sapp/reportesAdmision/generar?actaId={actaId}&convocatoriaId={convocatoriaId}` como respuesta binaria PDF, no como envelope JSON. El servicio devuelve un `Blob` con MIME `application/pdf`, toma el nombre desde `Content-Disposition` cuando exista y muestra debajo del formulario un PDF generado con icono, fecha Colombia y botones estandarizados **Ver**/**Descargar** (`sapp-document-action`). Matrícula y créditos condonables siguen usando el mock temporal.
