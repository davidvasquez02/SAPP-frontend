...
- **El tablero de estudiantes de coordinación es compacto y navegable:** `/coordinacion/estudiantes` conserva filtros, contador, estados y acceso al perfil, pero presenta cada estudiante como una tarjeta con portada de ancho completo y 240 px de alto, fotografía sin recorte o iniciales grandes, jerarquía de nombre/código/cohorte y una distinción textual e icónica entre activo e inactivo. El tablero mantiene scroll nativo y flechas con estado deshabilitado, suma teclado (`←`/`→`) y arrastre horizontal con mouse mediante Pointer Events; un umbral separa el clic del arrastre para que desplazar nunca abra un perfil.
...
- **2026-09-20:** se rediseñó `/coordinacion/estudiantes` como tablero horizontal institucional de tarjetas compactas, sin añadir datos académicos ni modificar servicios. Las fotos se redujeron a 64 px y el fallback usa iniciales; el resumen prioriza nombre, código UIS, cohorte y estado, mientras **Ver perfil** conserva la navegación y la caché existentes. El arrastre con botón izquierdo usa Pointer Events, captura de puntero y umbral de 6 px; ignora controles interactivos, cancela gestos verticales, suprime el clic posterior a un arrastre y no reemplaza el scroll nativo táctil/trackpad. Las flechas reflejan ambos límites y el contenedor responde a teclado. No cambiaron API, roles, paquetes, seeds ni datasets.
