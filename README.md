...
- **Aspirante creation flow:** `CreateAspiranteModal` posts `/sapp/aspirante`, stores the returned `aspiranteId` + `inscripcionAdmisionId`, and then uploads selected documents **sequentially** via `POST /sapp/document` using base64 + SHA-256 helpers. Until a requisitos endpoint exists, the modal uses a local document template for trámite `ADMISION_ASPIRANTE`, keeps the modal open on partial failures, and allows retrying failed uploads.
