...
- **Solicitudes cambio de estado (coordinador):** `src/modules/solicitudes/api/solicitudCambioEstadoService.ts` ahora usa el endpoint unificado `PUT /sapp/solicitudesAcademicas/cambioEstado/{solicitudId}?siglaEstado=...` para las acciones de detalle del coordinador (`EN_REVISION`, `APROBADA`, `RECHAZADA`), y el detalle recarga la solicitud después del cambio.
...
- **Solicitudes coordinación (detalle documentos):** `src/modules/solicitudes/api/solicitudDocumentosService.ts` consume `GET /sapp/document?tramiteId={solicitudId}&codigoTipoTramiteId={tipoSolicitudCodigo}` y mapea la respuesta del checklist a `DocumentosAdjuntos` para ver/descargar archivos cargados.
...
- April 8, 2026: en `/aspirante/documentos`, al completar la carga del **último documento obligatorio** durante una acción de subida del usuario (no en la carga inicial de pantalla), el frontend ahora ejecuta en background `PUT /sapp/inscripcionAdmision/cambioEstadoVal/{inscripcionId}` para pasar la inscripción a “por validar documentos”, sin feedback visual adicional para el aspirante.
